...
X_obs_2 = tsode[11:21, tri, g]  
X_inputs = tsode[11:21, tri, findall(x->x!=0,gs[:,g])] 
...
function regulated_spring!(du, u, p, t)
    ...
    du[1] = dX2
    du[2] = -β * dX2 - ω0^2 * (X2 - X_steady_2) + X_inputs[Int(floor(t))+1,:]' * W 
end
...
function loss_spring(params)
    ...
    u0 = [X_obs_2[1], 0.0]
    tspan = (0., 10.)
    prob = ODEProblem(regulated_spring!, u0, tspan, [β, ω0, X_steady_2, W...])
    sol = solve(prob, saveat=1.)
    X_pred = [sol[i][1] for i in 1:length(X_obs_2)]
    return sqrt(mean((X_obs_2 .- X_pred).^2))
end
...
u0 = [X_obs_2[1], 0.0]
tspan = (0, 10)
prob = ODEProblem(regulated_spring!, u0, tspan, optimized_params)
...
p = plot(t_obs .+ 10, X_obs_2, label="Observed", marker=:circle, lw = 4, ylims = [0, 1])
